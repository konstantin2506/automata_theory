...
TEST_CASE("Regex")
{
    ...
    SECTION("Cringe scenery")
    {
        ...
        CHECK(checker.checkLine("10 a = a --1", logger) == false);
        ...
        CHECK(checker.checkLine("3 c = 1--1", logger) == false);     // два минуса без пробела
        ...
        CHECK(checker.checkLine("8 h = 1+1", logger) == false);      // нет пробелов вокруг оператора
        CHECK(checker.checkLine("9 i = 1  +  1", logger) == true);   // много пробелов
        CHECK(checker.checkLine("10 j = 1 +1", logger) == false);    // пробел только слева от оператора
        CHECK(checker.checkLine("11 k=1+1", logger) == false);       // совсем без пробелов
